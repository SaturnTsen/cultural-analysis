...
from src.config import RESULTS_DIR, HOFSTEDE_DIMENSIONS, VISUALIZATION_CONFIG
...
class DataVisualizer:
    ...
    def __init__(self, analysis_results=None):
        ...
        self.colors = {
            ...
            'positive': VISUALIZATION_CONFIG['sentiment_colors']['positive'], # 绿色
            'neutral': VISUALIZATION_CONFIG['sentiment_colors']['neutral'],   # 灰色
            'negative': VISUALIZATION_CONFIG['sentiment_colors']['negative']  # 红色
        }
        
        # 设置Matplotlib样式
        plt.style.use('seaborn-v0_8-whitegrid')
        
    def visualize_all(self, analysis_results=None):
        """生成所有可视化"""
    ...
    def _save_visualization_results(self, results):
        """保存可视化结果"""
        results_file = os.path.join(self.results_dir, "visualization_results.json")
    ...
    def generate_word_clouds(self):
        ...
        for country in tf_idf_results.keys():
            ...
            for sector, sector_data in tf_idf_results[country].items():
                ...
                # 创建词-权重字典
                word_weights = {item['word']: item['score'] for item in sector_data['top_words']}
                ...
                plt.axis('off')
                plt.title(f"{country.capitalize()} - {sector.capitalize()} Sector", fontsize=16)
                plt.tight_layout()
                
                # 保存图像
                output_path = os.path.join(country_dir, f"wordcloud_{sector}.png")
    ...
    def generate_sentiment_charts(self):
        ...
        # 2. 创建国家间情感对比图
        country_sentiment = df.groupby(['country', 'sentiment'])['ratio'].mean().reset_index()
        ...
        # 保存图像
        output_path = os.path.join(self.viz_dir, "sentiment_country_comparison.png")
        plt.savefig(output_path, dpi=300, bbox_inches='tight')
        plt.close('all')  # 确保关闭所有图形
    ...
    def generate_topic_visualizations(self):
        ...
        for country in topic_results.keys():
            ...
            for sector, sector_data in topic_results[country].items():
                ...
                norm = Normalize(vmin=0, vmax=max([max(topic['weights']) for topic in topics]))
                ...
                # 调整布局
                plt.suptitle(f"{country.capitalize()} - {sector.capitalize()} Sector: Topic Model", fontsize=16)
                plt.tight_layout()
    ...
    def generate_cultural_radar_charts(self):
        ...
        for country in countries:
            ...
            scores = [cultural_results[country][dim]['score'] for dim in dimensions]
        ...
        # 保存图像
        output_path = os.path.join(self.viz_dir, "cultural_dimensions_radar.html")
    ...
    def generate_country_comparison_charts(self):
        ...
        if 'sentiment' in self.analysis_results:
            ...
            if sentiment_data:
                ...
                # 保存图表
                output_path = os.path.join(self.viz_dir, "france_china_sentiment_comparison.html")
                fig.write_html(output_path)
                
                # 也保存为图像
                img_path = os.path.join(self.viz_dir, "france_china_sentiment_comparison.png")
                # fig.write_image(img_path, width=800, height=600, scale=2)
