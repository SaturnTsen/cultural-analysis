...
class DataAnalyzer:
    ...
    def _load_corpus(self):
        ...
        for country in ['france', 'china']:
            ...
            for sector_dir in sector_dirs:
                ...
                # 加载预处理后的文本文件
                processed_files = glob(os.path.join(sector_dir, "processed_*.txt"))
                ...
                for processed_file in processed_files:
                    ...
                    try:
                        with open(processed_file, 'r', encoding='utf-8') as f:
                            ...
                            corpus[country]['processed_texts'][sector].append(processed_text)
                    except Exception as e:
                        logger.error(f"加载预处理文本文件{processed_file}时出错: {str(e)}")
                        continue
                    
                    # 加载分句文件
                    sentences_file = os.path.join(sector_dir, f"sentences_{base_filename}")
                    if os.path.exists(sentences_file):
                        try:
                            with open(sentences_file, 'r', encoding='utf-8') as f:
                                sentences = f.read().split('\n')
                                corpus[country]['sentences'][sector].append(sentences)
                        except Exception as e:
                            logger.error(f"加载分句文件{sentences_file}时出错: {str(e)}")
    ...
    def analyze_tf_idf(self):
        ...
        for country in self.corpus.keys():
            ...
            for sector, texts in self.corpus[country]['processed_texts'].items():
                sector_indices[sector] = (current_index, current_index + len(texts))
    ...
    def analyze_sentiment(self):
        ...
        for country in self.corpus.keys():
            ...
            for sector, sentences_list in self.corpus[country]['sentences'].items():
                ...
                # 扁平化句子列表
                all_sentences = [sentence for doc_sentences in sentences_list for sentence in doc_sentences if sentence.strip()]
                ...
                sample_sentences = np.random.choice(all_sentences, sample_size, replace=False)
                
                # 分析情感
                sentiments = self._analyze_sentences_sentiment(sample_sentences, country)
    ...
    def _analyze_sentences_sentiment(self, sentences, country):
        ...
        if lang not in self.sentiment_models:
            try:
                ...
                self.sentiment_models[lang] = pipeline('sentiment-analysis', model=model, tokenizer=tokenizer)
            except Exception as e:
                logger.error(f"加载{lang}情感分析模型时出错: {str(e)}")
                
                # 出错时使用英文模型代替
                if 'en' not in self.sentiment_models:
                    tokenizer = AutoTokenizer.from_pretrained(ANALYSIS_CONFIG['sentiment']['transformer_model']['en'])
                    model = AutoModelForSequenceClassification.from_pretrained(ANALYSIS_CONFIG['sentiment']['transformer_model']['en'])
                    self.sentiment_models['en'] = pipeline('sentiment-analysis', model=model, tokenizer=tokenizer)
    ...
    def analyze_cultural_dimensions(self):
        ...
        for country in self.corpus.keys():
            ...
            for dimension in HOFSTEDE_DIMENSIONS[country].keys():
                ...
                if dimension == 'power_distance':
                    keywords_high = ['hierarchy', 'authority', 'power', 'order', 'obedience']
                    keywords_low = ['equality', 'flat', 'democratic', 'participation', 'consensus']
                elif dimension == 'individualism':
                    keywords_high = ['individual', 'personal', 'unique', 'self', 'independent']
                    keywords_low = ['collective', 'group', 'team', 'cooperation', 'harmony']
                # ... 其他维度的关键词
                
                # 计算关键词频率
                high_score = sum(combined_text.count(word) for word in keywords_high)
                low_score = sum(combined_text.count(word) for word in keywords_low)
                
                # 如果有足够的关键词匹配
                if high_score + low_score > 10:
                    # 计算偏好比例
                    ratio = high_score / (high_score + low_score) if high_score + low_score > 0 else 0.5
                    ...
                    # 计算最终分数，介于0-100之间
                    dimension_score = base_score * (1 - weight) + 100 * ratio * weight
                else:
                    # 如果没有关键词匹配，使用标准分数
        ...
        # 保存结果
        results_file = os.path.join(self.results_dir, "cultural_dimensions_results.json")
        with open(results_file, 'w', encoding='utf-8') as f:
            json.dump(results, f, ensure_ascii=False, indent=2)
